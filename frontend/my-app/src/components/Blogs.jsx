...
export default function Blogs() {
  ...
  const [commentTexts, setCommentTexts] = useState({}); // ✅ Per-blog comment inputs
  ...
  const fetchBlogs = async () => {
    ...
    try {
      const response = await axios.get(
        `${BLOGS_API_URL}?limit=${LIMIT}&offset=${offset}`
      );
      ...
      if (Array.isArray(response.data)) {
        ...
        setTotalBlogs(response.data.count || 0);
      }
    } catch (error) {
  ...
  const handleAddComment = async (e, blogId) => {
    e.preventDefault();
    if (!commentTexts[blogId] || !commentTexts[blogId].trim()) {
      toast.warn("Comment cannot be empty.");
      return;
    }
    try {
      const newComment = { comment: commentTexts[blogId], blog: blogId };
    ...
    <div className="blogs-container">
          ...
          <Accordion.Body>
            <Form onSubmit={handleAddBlog}>
              ...
              <Form.Control
                ...
                onChange={(e) =>
                  setNewBlog({ ...newBlog, blog_body: e.target.value })
                }
                required
                className="mb-3"
              />
      ...
      {totalBlogs > LIMIT && (
        <div className="d-flex justify-content-center align-items-center my-4">
          ...
          <Button
            ...
            disabled={offset + LIMIT >= totalBlogs}
            className="ms-2"
          >
      ...
      <Modal show={showBlogModal} onHide={closeModals} centered>
        ...
        <Modal.Footer>
          ...
          <Button className="btn-gradient" onClick={handleSaveBlog}>
            Save Changes
          </Button>
        </Modal.Footer>
      </Modal>
        ...
        <Modal.Footer>
          ...
          <Button
            ...
            onClick={async () => {
              try {
                await axios.put(
                  `${COMMENTS_API_URL}${editingComment.id}/`,
                  editingComment
                );
